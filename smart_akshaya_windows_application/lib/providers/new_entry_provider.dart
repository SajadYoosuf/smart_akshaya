...
class NewEntryProvider extends ChangeNotifier {
  ...
  // Wallets from Google Sheets
  List<String> walletTypes = ['Cash', 'BANK', 'Edistrict', 'CSC', 'UPI', 'UTI'];
      ...
      if (rows.length > 1) {
        ...
        if (nameIdx != -1) {
          ...
          for (int i = 1; i < rows.length; i++) {
            final r = rows[i];
            if (r.length > nameIdx && r[nameIdx].toString().trim().isNotEmpty) {
              list.add(r[nameIdx].toString().trim());
            }
          }
          if (list.isNotEmpty) {
            walletTypes = list;
            if (!walletTypes.contains(selectedWallet)) {
              selectedWallet = walletTypes[0];
            }
          }
        }
      }
    } catch (e) {
      print('Error loading wallets in entry: $e');
    }
  }
  ...
  void editService(int index) {
    ...
    selectedWallet = item.walletName;
  ...
  Future<void> saveEntry(
    ...
    // Credit / Walk-in Customer validation
